...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[os.getenv("FRONTEND_URL", "*")], # In production limit this to seat sync domains
...
def init_firebase():
    ...
    raise ValueError("No Firebase Admin credentials provided. Set FIREBASE_SERVICE_ACCOUNT_JSON or PROJECT_ID/EMAIL/PRIVATE_KEY.")

try:
    init_firebase()
    db = firestore.client()
except Exception as e:
    logger.warning(f"Could not initialize Firebase Admin at startup (normal during build/CI): {e}")


async def verify_auth(authorization: str = Header(...)):
    """Very simple auth middleware checking the Bearer Firebase ID token."""
...
def get_user_fcm_tokens(user_id: str) -> List[str]:
    ...
    tokens = []
    try:
        # Check if stored as a single field
...
def send_fcm_multicast(user_id: str, title: str, body: str, data: dict = None, link: str = None) -> bool:
    ...
    # We use multicast to send to multiple devices for the same user concurrently
    ...
    try:
        ...
        if response.failure_count > 0:
            ...
            for idx, resp in enumerate(response.responses):
                if not resp.success:
                    # Common errors: exception.code == 'messaging/invalid-registration-token'
...
@app.get("/health")
def health_check():
    return {"status": "ok", "app": "SeatSync Notification API"}

@app.post("/api/tokens/register")
def register_token(req: TokenRegistration, user: dict = Depends(verify_auth)):
    uid = user['uid']
    user_ref = db.collection('users').document(uid)
    db.run_transaction(lambda t: _register_token_transaction(t, user_ref, req.token))
    return {"status": "success", "message": "Token registered"}

@firestore.transactional
def _register_token_transaction(transaction, user_ref, token):
    ...
    if token not in tokens:
        tokens.append(token)
        # We also set fcmToken (singular) for backward compatibility
        transaction.update(user_ref, {'fcmTokens': tokens, 'fcmToken': token})
...
def notify_payment_status(req: PaymentStatusNotification, user: dict = Depends(verify_auth)):
    # Coordinator/Admin sends this to a User
...
def notify_event_update(req: EventUpdateNotification, user: dict = Depends(verify_auth)):
    # Coordinator/Admin updates an event, notify all registered users
    # We fetch registered users from DB directly to ensure correctness and prevent tampering
...
def notify_generic(req: GenericNotification, user: dict = Depends(verify_auth)):
    # Verify the sender is an admin or coordinator to prevent abuse
    # Or simply restrict this endpoint if not strictly necessary. We will allow it for simplicity but log heavily.
