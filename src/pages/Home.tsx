...
export const Home: React.FC = () => {
  ...
  // Calculate lowest non-zero fee for dynamic pricing display
  const lowestFee = events.reduce((min, ev) => {
    if (ev.registration_fee && ev.registration_fee > 0) {
      return min === 0 ? ev.registration_fee : Math.min(min, ev.registration_fee);
    }
    return min;
  }, 0);

  const pricingText = lowestFee > 0 ? `Starting from ₹${lowestFee}` : 'Starting from ₹100';
  ...
  return (
    <div className="min-h-screen bg-transparent text-[#1D1D1F] flex flex-col justify-between" style={{ fontFamily: '"DM Sans", sans-serif' }}>
      <div>
        ...
        {/* Hero Section */}
        <section className="max-w-[1100px] mx-auto px-4 sm:px-6 pt-12 sm:pt-20 pb-12 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/70 backdrop-blur-md border border-white/90 shadow-sm mb-6">
            ...
            <span className="text-[12px] font-extrabold text-[#1D1D1F] uppercase tracking-wider">Official SIMATS Portal</span>
          </div>

          <h1 className="font-extrabold text-[36px] sm:text-[56px] text-[#1D1D1F] tracking-tight leading-[1.1] mb-6">
            Discover & Book <br />
            <span className="bg-gradient-to-r from-[#3B9EFF] to-[#007AFF] bg-clip-text text-transparent">
              Academic Workshops & Events
            </span>
          </h1>

          <p className="max-w-2xl mx-auto text-[#5E6C84] text-[15px] sm:text-[18px] font-medium leading-relaxed mb-8">
          ...
          {/* Pricing Highlight & CTA Box */}
          <div className="inline-flex flex-col sm:flex-row items-center gap-4 p-2 bg-white/70 backdrop-blur-2xl rounded-[32px] sm:rounded-full border border-white shadow-[0_12px_40px_rgba(0,100,200,0.08)] mb-8 max-w-lg mx-auto">
            <div className="px-5 py-2 flex items-center gap-2">
              <span className="text-[14px] font-extrabold text-[#1D1D1F]">{pricingText}</span>
              <span className="text-xs text-[#5E6C84]">• Instant Access</span>
            </div>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <Link
                to="/register"
                className="w-full sm:w-auto px-7 py-3.5 rounded-full bg-[#1D1D1F] hover:bg-black text-white font-extrabold text-[14px] transition-all shadow-md active:scale-95 text-center"
              >
                Enroll Now
              </Link>
              <Link
                to="/login"
                className="w-full sm:w-auto px-6 py-3.5 rounded-full bg-white/80 hover:bg-white text-[#1D1D1F] font-bold text-[14px] transition-all border border-white shadow-sm active:scale-95 text-center"
              >
                Sign In
              </Link>
            </div>
        ...
        <section className="max-w-[1100px] mx-auto px-4 sm:px-6 relative z-10 mb-20">
                  ...
                  <div key={event.id} className="bg-white/80 backdrop-blur-2xl rounded-[32px] p-4 shadow-[0_12px_40px_rgba(0,100,200,0.08)] hover:shadow-[0_18px_50px_rgba(0,100,200,0.12)] transition-all border border-white flex flex-col group">
                    ...
                    <div className="px-1 flex flex-col gap-3 flex-1">
                      ...
                      <div className="flex justify-between items-start gap-3">
                        ...
                        {/* Glassy Arrow Button */}
                        <Link to={`/event/${event.id}`} aria-label={`View ${event.title}`} className="w-10 h-10 bg-[#1D1D1F]/90 backdrop-blur-md rounded-full flex items-center justify-center shrink-0 shadow-sm transition-transform hover:scale-105 active:scale-95">
      ...
      {/* Modern Footer with Copyright 2026 & Clickable Contact Links */}
      <footer className="w-full max-w-[1100px] mx-auto px-4 sm:px-6 py-8 border-t border-white/40 mt-12 relative z-10">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left text-[#5E6C84] text-[13px] font-medium">
          <div className="flex items-center gap-2">
            <span className="font-extrabold text-[#1D1D1F]">SeatSync</span>
            <span>•</span>
            <span>SIMATS Engineering</span>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a href="mailto:support@simats.edu" className="hover:text-[#1D1D1F] transition-colors">
              support@simats.edu
            </a>
            <span>•</span>
            <a href="tel:+914426801999" className="hover:text-[#1D1D1F] transition-colors">
              +91 44 2680 1999
            </a>
          </div>
          <div>
