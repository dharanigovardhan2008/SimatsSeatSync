...
const charsOf = (raw: string | undefined) => String(raw ?? '').replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
const toSlots = (raw: string | undefined, n: number) => {
  const d = charsOf(raw).slice(0, n);
...
const CodeSlots: React.FC<CodeSlotsProps> = ({
  ...
  const emitted = useRef(charsOf(value ?? defaultValue).slice(0, length));
  ...
  const onInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (busy) return;
    const d = charsOf(e.target.value);
    if (!d) return;
  ...
  useEffect(() => {
    ...
    const clean = charsOf(value).slice(0, length);
